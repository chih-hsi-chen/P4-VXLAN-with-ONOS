...
MY_PIPECONF = "nctu.pncourse.pipeconf"
    ...
    try:
        ...
        while True:
            ...
            with closing(socket.socket(socket.AF_INET, socket.SOCK_STREAM)) as s:
                port = sw.grpcPortInternal if sw.grpcPortInternal else sw.grpc_port
...
class ONOSBmv2Switch(Switch):
    ...
    def __init__(self, name, sw_path = None, json_path = None, json=None, debugger=False, loglevel="warn",
                 elogger=False, grpc_port=None, cpuport=255, notifications=False,
                 thrift_port=None, netcfg=True, dryrun=False,
                 pipeconf=MY_PIPECONF, pktdump=False, pcap_dump = False, valgrind=False,
                 gnmi=False, portcfg=True, onosdevid=None, stratum=False, log_console = False,
        ...
        self.device_id = BMV2_DEFAULT_DEVICE_ID
        self.logfd = None
    ...
    def getDeviceConfig(self, srcIP):
        ...
        basicCfg = {
            "managementAddress": "grpc://%s:%d?device_id=%d" % (
                srcIP, self.grpc_port, self.device_id),
...
nodes {{
  ...
  slot: 1
  index: 1
}}\n""".format(name=self.name, nodeId=self.device_id)
  ...
  node: {nodeId}
    ...
    def start(self, controllers):
        ...
        if self.grpc_port is None:
            self.grpc_port = pickUnusedPort()
        writeToFile("/tmp/bmv2-%s-grpc-port" % self.name, self.grpc_port)
        ...
        if self.useStratum:
            ...
            if self.thrift_port is None:
                self.thrift_port = pickUnusedPort()
            writeToFile("/tmp/bmv2-%s-thrift-port" % self.name, self.thrift_port)
    ...
    def getStratumCmdString(self, config_dir):
        ...
        args = [
            ...
            '-external_hercules_urls=0.0.0.0:%d' % self.grpc_port,
    ...
    def bmv2Args(self):
        args = ['--device-id %s' % str(self.device_id)]
        for port, intf in self.intfs.items():
            if not intf.IP():
                args.append('-i %d@%s' % (port, intf.name))
        args.append('--thrift-port %s' % self.thrift_port)
        ...
        args.append('--grpc-server-addr 0.0.0.0:%s' % self.grpc_port)
        return args
    ...
    def waitBmv2Start(self):
        ...
        while True:
            port = self.grpcPortInternal if self.grpcPortInternal else self.grpc_port
